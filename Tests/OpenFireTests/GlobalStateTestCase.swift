...
class GlobalStateTestCase: XCTestCase {
    ...
    override func setUp() {
        ...
        let lockPath = "/tmp/com.openfire.tests.\(getuid()).global-state.lock"
        fileDescriptor = lockPath.withCString {
            open($0, O_CREAT | O_RDWR | O_CLOEXEC | O_NOFOLLOW, S_IRUSR | S_IWUSR)
        }
        precondition(fileDescriptor >= 0, "Unable to open the OpenFire test isolation lock.")
        precondition(flock(fileDescriptor, LOCK_EX) == 0, "Unable to acquire the OpenFire test isolation lock.")
    }
