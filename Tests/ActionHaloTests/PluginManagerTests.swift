...
final class PluginManagerTests: GlobalStateTestCase {
    ...
    func testPresentationPluginsKeepsAllEnabledPluginsVisible() {
        ...
        let openURLJSON = #"{"name":"Open URL","identifier":"com.actionhalo.open-url","action":{"type":"url","url":"{text}"},"icon":"link","order":70,"filter":{"minLength":5,"regex":"^(https?://|www\\.)[^ ]+"}}"#
        let revealPathJSON = #"{"name":"Reveal Path","identifier":"com.actionhalo.reveal-path","action":{"type":"reveal-path"},"icon":"folder","order":65,"filter":{"minLength":2,"regex":"^/.*$"}}"#
        ...
        XCTAssertEqual(available.map(\.id), ["com.test.search", "com.actionhalo.reveal-path", "com.actionhalo.open-url"])
    }
    ...
    func testMergePluginsPreservingExistingKeepsBuiltInCorePluginWhenUserUsesCoreIdentifier() {
        ...
        let builtInCopy = makePlugin(
            ...
            identifier: "com.actionhalo.copy",
            order: 1,
            directory: "/Applications/ActionHalo.app/Contents/Resources/Plugins/Copy.actionhaloext"
        )
        ...
        XCTAssertEqual(merged.count, 1)
        XCTAssertEqual(merged.first?.id, "com.actionhalo.copy")
    ...
    func testFilterSoftDeletedBuiltInsKeepsUserOverrideAndRemovesBundledCopy() {
        let userPlugin = makePlugin(
            ...
            order: 1,
            directory: "/Users/me/Library/Application Support/ActionHalo/Plugins/Search.actionhaloext"
        )
        let builtInDeletedPlugin = makePlugin(
            name: "Built In Search",
            identifier: "com.test.deleted",
            order: 2,
            directory: "/Applications/ActionHalo.app/Contents/Resources/Plugins/Search.actionhaloext"
        )
        ...
        let filtered = PluginManager.filterSoftDeletedBuiltIns(
            ...
            deletedBuiltInPluginIDs: ["com.test.search", "com.test.deleted", "com.actionhalo.copy"],
            builtInPluginsURL: URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/Plugins")
        )

        XCTAssertEqual(filtered.map(\.id), ["com.test.search", "com.actionhalo.copy"])
    }
    ...
    func testFilterSoftDeletedBuiltInsDoesNotTreatSiblingPathPrefixAsBuiltIn() {
        let siblingPlugin = makePlugin(
            ...
            order: 1,
            directory: "/Applications/ActionHalo.app/Contents/Resources/PluginsBackup/Search.actionhaloext"
        )
    ...
    func testPluginDirectoryContainmentRequiresDirectoryBoundary() {
        let pluginsURL = URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/Plugins")

        XCTAssertTrue(PluginManager.isPluginDirectory(
            URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/Plugins/Search.actionhaloext"),
            inside: pluginsURL
        ))
        XCTAssertFalse(PluginManager.isPluginDirectory(
            URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/PluginsBackup/Search.actionhaloext"),
            inside: pluginsURL
        ))
        XCTAssertFalse(PluginManager.isPluginDirectory(
            URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/Plugins"),
            inside: pluginsURL
        ))
    }
    ...
    func testPluginDirectoryContainmentRejectsSymlinkEscape() throws {
        ...
        let symbolicLinkURL = parentURL.appendingPathComponent("Escape.actionhaloext")
    ...
    func testBuiltInPluginDirectoryUsesConfiguredBuiltInDirectory() {
        let builtInPluginsURL = URL(fileURLWithPath: "/Applications/ActionHalo.app/Contents/Resources/Plugins")
        ...
        XCTAssertFalse(PluginManager.isBuiltInPluginDirectory(
            URL(fileURLWithPath: "/tmp/Other.app/Contents/Resources/Plugins/Search.actionhaloext"),
            builtInPluginsURL: builtInPluginsURL
        ))
    }

    func testVisibleUserPluginFileNameDoesNotCreateHiddenPackageNames() {
        XCTAssertEqual(PluginManager.visibleUserPluginFileName(for: ".book"), "book.actionhaloext")
    ...
    func testPluginIdentifierValidationMatchesEditorRulesForNewPlugins() {
        ...
        XCTAssertEqual(
            PluginManager.pluginIdentifierValidationMessage("COM.ACTIONHALO.COPY"),
    ...
    func testRepairHiddenUserPluginPackagesLeavesInvalidPackageQuarantined() throws {
        ...
        let hiddenPackageURL = pluginsURL.appendingPathComponent(".book.actionhaloext")
        let visiblePackageURL = pluginsURL.appendingPathComponent("book.actionhaloext")
    ...
    func testWatchablePluginDirectoriesIncludesExistingPluginPackages() throws {
        ...
        let packageURL = pluginsURL.appendingPathComponent("Custom.actionhaloext")
        let regularDirectoryURL = pluginsURL.appendingPathComponent("Notes")
        let fakePackageFileURL = pluginsURL.appendingPathComponent("NotADirectory.actionhaloext")
        ...
        XCTAssertEqual(watchableNames, [pluginsURL.lastPathComponent, "Custom.actionhaloext"])
    }
    ...
    func testUserPluginURLFindsLegacyNamedOverrideByIdentifier() throws {
        let manager = PluginManager.shared
        let legacyURL = manager.userPluginsURL.appendingPathComponent("Legacy Name.actionhaloext")
        ...
        XCTAssertEqual(resolvedURL?.lastPathComponent, "Legacy Name.actionhaloext")
    }
    ...
    func testRemoveDuplicateUserPluginsPreservesNewlySavedPackageURL() throws {
        ...
        let packageURL = manager.userPluginsURL.appendingPathComponent("\(identifier).actionhaloext")
    ...
    func testInstallPluginRejectsInvalidPluginPackage() throws {
        let manager = PluginManager.shared
        let invalidURL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString + ".actionhaloext")
    ...
    func testInstallPluginRejectsCorePluginIdentifierOverride() throws {
        let manager = PluginManager.shared
        let pluginURL = try makePluginBundle(identifier: "com.actionhalo.copy", name: "Fake Copy")
        let destinationURL = manager.userPluginsURL.appendingPathComponent("com.actionhalo.copy.actionhaloext")
        temporaryDirectories.append(destinationURL)
    ...
    func testInstallPluginRejectsExternalPackageWithUnsafeIdentifier() throws {
        ...
        let destinationURL = manager.userPluginsURL.appendingPathComponent("z-lib.actionhaloext")
        temporaryDirectories.append(destinationURL)
    ...
    func testInstallPluginReplacesExistingPackageWithValidatedStagedCopy() throws {
        ...
        let destinationURL = manager.userPluginsURL.appendingPathComponent("\(identifier).actionhaloext")
        temporaryDirectories.append(destinationURL)
        ...
        XCTAssertFalse(
            (try FileManager.default.contentsOfDirectory(atPath: manager.userPluginsURL.path))
                .contains { $0.hasSuffix(".actionhaloext.pending") }
        )
    }
    ...
    func testRecoverInterruptedPluginOperationsRestoresBackupAndRemovesStaging() throws {
        ...
        let backupURL = manager.userPluginsURL.appendingPathComponent(
            ".backup-test.actionhaloext.pending"
        )
        try FileManager.default.createDirectory(
            at: backupURL,
            withIntermediateDirectories: true
        ...
        let stagingURL = manager.userPluginsURL.appendingPathComponent(
            ".install-test.actionhaloext.pending"
        )
        try FileManager.default.createDirectory(
            at: stagingURL,
            withIntermediateDirectories: true
        ...
        let restoredURL = manager.userPluginsURL.appendingPathComponent(
            "com.test.recovered.actionhaloext"
        )
    ...
    func testRestoreBackedUpPackageMovesBackupWhenDestinationIsMissing() throws {
        ...
        let backupURL = parentURL.appendingPathComponent(".backup-test.actionhaloext.pending")
        let destinationURL = parentURL.appendingPathComponent("Plugin.actionhaloext")
    ...
    func testPluginProcessRunnerCleansBackgroundChildHoldingStderrWithoutBlocking() throws {
        ...
        process.arguments = [
            "-c",
            """
            printf '%s' "$ACTIONHALO_TEST_VALUE" > runner-context.txt
            sleep 5 >&2 &
            echo $! > child.pid
            """
        ]
        ...
        environment["ACTIONHALO_TEST_VALUE"] = "preserved"
    ...
    func testAppleScriptTextInputOnlyRewritesActionHaloEnvironmentName() {
        ...
        let rendered = PluginManager.replacingPluginTextEnvironmentReferences(
            in: source,
            with: "read selectionFile as «class utf8»"
        )
    ...
    func testBuiltInRunShellReadsBinaryUnsafeSelectionFromTextFile() throws {
        ...
        let scriptURL = repositoryRoot.appendingPathComponent(
            "Plugins/Run Shell.actionhaloext/script.sh"
        )
        let process = Process()
        ...
        environment["ACTIONHALO_TEXT_FILE"] = textFile.path
        environment.removeValue(forKey: "ACTIONHALO_TEXT")
        process.environment = environment
        ...
        var expectedLog = Data("ActionHalo triggered this script!\nSelected text was: ".utf8)
        expectedLog.append(selectedText)
        expectedLog.append(Data("\n".utf8))
        XCTAssertEqual(
            try Data(contentsOf: desktopDirectory.appendingPathComponent("actionhalo.log")),
            expectedLog
        )
    }
    ...
    func testDeletePluginUsesActualContainedDirectoryInsteadOfIdentifierPath() throws {
        ...
        let outsideVictimURL = manager.userPluginsURL
            .deletingLastPathComponent()
            .appendingPathComponent("victim.actionhaloext")
