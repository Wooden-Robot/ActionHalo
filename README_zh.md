...
✂️ **更干净的快捷操作**
当 OpenFire 检测到空白输入框时，可以在光标附近弹出轻量的 `Paste / Clear` 胶囊操作条，适合快速粘贴或清空输入内容。内置 `粘贴` 动作现在默认也通过这里触发，而不是放进选中文本后的主圆环。
...
先记住一条总规则：**圆环一定是在你完成文字选择并松开鼠标之后才出现的**。

触发之后有两种使用方式：

1. **选中，松开，再点击**：先完成框选，圆环弹出后，直接点击目标扇区执行动作。
2. **选中，松开，再按下拖拽，最后松开**：先完成框选并触发圆环，然后立刻按下鼠标，拖到目标扇区，松开时执行动作。

第二种是 OpenFire 很有辨识度的交互方式：它不是“选中文本和执行动作一次手势完成”，而是“圆环出现后立刻进入按下-拖拽-松开”的武器轮盘式操作。
